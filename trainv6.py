# ==============================================================
# PPO训练脚本 - 版本5 + BadSeed课程采样版 + Terminal Critic Replay修正版
# 本次改进目标平均分数300+
# 说明：
...
#    - 学习率调度（线性衰减）
#    - KL early stop
#    - 【新增】badseed 课程采样（前期少、后期多）
#    - 【新增】terminal critic replay（仅训练critic头）
# ==============================================================
# 【本次最小改动说明】
# 1. 保留原 PPO 主体结构、训练逻辑、损失计算逻辑不变
# 2. 保留 badseed curriculum 逻辑不变
# 3. 修复 terminal replay 的几个关键问题：
#    - 修复 replay buffer 在 episode 结束时被提前清空的问题
#    - 修复跨 episode 数据污染问题
#    - replay 时仅训练 critic，不更新 actor / shared
# 4. 在修改处保留原代码注释，并标明【修改】/【新增】
# ==============================================================
...
# 【新增】单独给 terminal replay 一个 critic optimizer
# 这样 replay 时不会误伤 actor / shared
    ...
    if use_terminal_replay and terminal_replay.size() >= terminal_replay_batch:
        ...
        for _ in range(terminal_replay_epochs):
            ...
            torch.nn.utils.clip_grad_norm_(
                model.critic.parameters(), max_grad_norm)
