...
# 【本次修改目标】
# 1. 删去共享层
# 2. Actor / Critic 改为完全独立网络
# 3. 激活函数由 ReLU 改为 Tanh
# 4. 尽量保持原有训练流程、badseed课程采样、terminal replay不变
# ==============================================================
...
class model_v1(nn.Module):
    ...
    def forward(self, state):
        # ==========================================================
        # 【修改】原代码先走 shared，再分别进 actor / critic
        #
        # 原：
...
# ==============================
# 【新增】Terminal Replay Buffer
# 用于存储多个 episode 的末端窗口数据，供 critic replay 使用
# 注意：这是“全局重放缓冲区”
# ==============================
class terminal_replay_buffer:
    ...
    def sample(self, batch):
        idx = np.random.choice(len(self.states), batch, replace=False)
        return (
            np.array([self.states[i] for i in idx], dtype=np.float32),
            np.array([self.rewards[i] for i in idx], dtype=np.float32),
            np.array([self.next_states[i] for i in idx], dtype=np.float32),
            np.array([self.dones[i] for i in idx], dtype=np.float32)
        )
...
def reset_game_with_seed(game, seed=None):
    ...
    注意：
    若你的 game 封装本身支持 seed 参数，这里即可正常工作；
    若不支持，你只需在 game.reset 内部补一个 seed 参数即可。
    '''
    if seed is None:
        return game.reset()
...
# 超参数
actor_lr = 3e-4             # 初始actor学习率
critic_lr = 2e-4            # 初始critic学习率
actor_lr_end = 3e-5         # 最终actor学习率
critic_lr_end = 5e-5        # 最终critic学习率
gamma = 0.99                # 折扣因子
...
# ==============================
# 【新增】Terminal Replay参数
# ==============================
use_terminal_replay = True       # 是否启用terminal replay
terminal_window = 40             # 只取每个badseed episode最后40步
terminal_replay_epochs = 2       # 每次update replay次数
terminal_replay_batch = 128      # replay batch size
terminal_replay_max_size = 20000  # 全局replay buffer最大容量
# ==============================

# 三段式课程采样参数：
# 前 30% 训练：badseed 比例固定为 5%
# 中间 40% 训练：从 5% 线性升到 30%
# 最后 30% 训练：从 30% 线性升到 35%
badseed_warmup_end = 0.30
badseed_ramp_end = 0.70
badseed_ratio_start = 0.05
badseed_ratio_mid = 0.30

# 【修改】原：
# badseed_ratio_end = 0.50
# 改为更稳的后期 badseed 占比，避免分布漂移过强
badseed_ratio_end = 0.35
...
# 【新增】全局 terminal replay buffer 实例
terminal_replay = terminal_replay_buffer(max_size=terminal_replay_max_size)
...
# 改为：
# - 删除 shared 参数组
# - actor_net 和 critic_net 分别独立优化
# ==========================================================
optimizer = torch.optim.Adam([
    {"params": model.actor_net.parameters(), "lr": actor_lr},    # 参数组0：actor
    {"params": model.critic_net.parameters(), "lr": critic_lr},  # 参数组1：critic
])

# ==========================================================
# 【修改】单独给 terminal replay 一个 critic optimizer
#
# 原代码：
# critic_replay_optimizer = torch.optim.Adam(
#     model.critic.parameters(),
#     lr=critic_lr
# )
#
# 改为：
# critic replay 仅更新 critic_net
# ==========================================================
critic_replay_optimizer = torch.optim.Adam(
    model.critic_net.parameters(),
    lr=critic_lr
)
...
# state = game.reset()
#
# 改为：首次 episode 也按课程采样逻辑 reset
# progress=0.0 对应训练开始阶段
# ==============================================================
state, current_episode_seed, current_episode_seed_source, current_badseed_ratio = sample_reset_state(
    game=game,
    progress=0.0,
...
# ==============================
# 【新增】当前 episode 的临时 terminal 缓存
# 注意：这是“当前局缓存”，不是全局 replay buffer
# ==============================
current_episode_terminal_states = []
current_episode_terminal_rewards = []
current_episode_terminal_next_states = []
current_episode_terminal_dones = []
# ==============================
...
for update_idx in range(updates):
    ...
    # ==============================
    # 【新增】记录当前 update 下的 badseed 课程采样比例
    # 这里只是记录目标比例，不影响 PPO 训练主体逻辑
    # ==============================
    ...
    # 参数组1：critic_net
    # ==========================================================
    optimizer.param_groups[0]["lr"] = current_actor_lr   # actor 学习率
    optimizer.param_groups[1]["lr"] = current_critic_lr  # critic 学习率

    # 【新增】同步 critic replay optimizer 学习率
    critic_replay_optimizer.param_groups[0]["lr"] = current_critic_lr
    ...
    while collected_steps < rollout_steps:
        ...
        # ==============================
        # 【修改】原代码：
        # if use_terminal_replay and current_episode_seed_source == "badseed":
        #     terminal_buffer.store(state, reward, next_state, done)
        #
        # 改为：
        # 先写入“当前 episode 临时缓存”，episode 结束后只截取最后 terminal_window 步
        # 再统一写入全局 terminal replay buffer
        # ==============================
        if use_terminal_replay and current_episode_seed_source == "badseed":
            current_episode_terminal_states.append(state)
            current_episode_terminal_rewards.append(reward)
            current_episode_terminal_next_states.append(next_state)
            current_episode_terminal_dones.append(done)
        # ==============================
        ...
        if done:
            ...
            # ==============================
            # 【新增】记录当前 episode 是否来自 badseed
            # ==============================
            episode_seed_source_history.append(current_episode_seed_source)
            # ==============================

            # ==============================
            # 【新增】episode 结束时，把当前 badseed episode 的最后 terminal_window 步
            # 写入全局 terminal replay buffer
            # ==============================
            if use_terminal_replay and current_episode_seed_source == "badseed":
                start_idx = max(
                    0, len(current_episode_terminal_states) - terminal_window)

                for i in range(start_idx, len(current_episode_terminal_states)):
                    terminal_replay.store(
                        current_episode_terminal_states[i],
                        current_episode_terminal_rewards[i],
                        current_episode_terminal_next_states[i],
                        current_episode_terminal_dones[i]
                    )
            # ==============================
            ...
            # state = game.reset()
            #
            # 改为：episode 结束后，按当前训练进度重新采样普通 seed / badseed
            # ==============================================================
            state, current_episode_seed, current_episode_seed_source, current_badseed_ratio = sample_reset_state(
                game=game,
                progress=progress,
            ...
            # ==============================
            # 【新增】清空当前 episode 临时 terminal 缓存
            # 这是“当前局缓存”，每局结束后必须清空
            # ==============================
            current_episode_terminal_states = []
            current_episode_terminal_rewards = []
    ...
    # ============================================================
    # 【新增】Terminal Critic Replay
    # 只更新critic网络，不更新actor
    # ============================================================
    if use_terminal_replay and terminal_replay.size() >= terminal_replay_batch:

        critic_loss_fn = nn.SmoothL1Loss()

        for _ in range(terminal_replay_epochs):

            s, r, ns, d = terminal_replay.sample(terminal_replay_batch)
            ...
            # ======================================================
            # 【修改】原代码 replay 时依赖 shared：
            #
            # with torch.no_grad():
            #     shared_ns = model.shared(ns)
            #     next_v = model.critic(shared_ns).squeeze(-1)
            #     target = r + gamma * next_v * (1 - d)
            #     target = torch.clamp(target, -10, 10)
            #
            # with torch.no_grad():
            #     shared_s = model.shared(s)
            #
            # value = model.critic(shared_s.detach()).squeeze(-1)
            #
            # 改为：
            # 由于已删除 shared，critic_net 直接输入状态
            # replay 只更新 critic_net，不影响 actor_net
            # ======================================================
            with torch.no_grad():
                next_v = model.critic_net(ns).squeeze(-1)
                target = r + gamma * next_v * (1 - d)
                target = torch.clamp(target, -10, 10)

            value = model.critic_net(s).squeeze(-1)

            replay_critic_loss = critic_loss_fn(value, target)

            critic_replay_optimizer.zero_grad()
            replay_critic_loss.backward()
            torch.nn.utils.clip_grad_norm_(
                model.critic_net.parameters(), max_grad_norm)
            critic_replay_optimizer.step()
    # ============================================================

    terminal_replay_size_history.append(terminal_replay.size())
    ...
    torch.save(
        {
            ...
            "critic_replay_optimizer_state_dict": critic_replay_optimizer.state_dict(),
            ...
            # 【新增】保存 terminal replay 相关配置
            "use_terminal_replay": use_terminal_replay,
            "terminal_window": terminal_window,
            "terminal_replay_epochs": terminal_replay_epochs,
            "terminal_replay_batch": terminal_replay_batch,
            "terminal_replay_max_size": terminal_replay_max_size,
            # ==============================
        },
        ...
        torch.save(
            {
                ...
                # ==============================
                # 【新增】保存 badseed 课程采样相关信息，方便回溯最佳模型训练配置
                # ==============================
                "use_badseed_curriculum": use_badseed_curriculum,
                "badseed_csv_path": badseed_csv_path,
                "badseed_ratio_start": badseed_ratio_start,
    ...
    print(
        ...
        f"TerminalReplaySize: {terminal_replay.size()}"
    )
...
# 学习率曲线
plt.figure(figsize=(10, 5))
plt.plot(actor_lr_history, label="Actor LR")
...
# ==============================
# 【新增】terminal replay buffer 大小曲线
# ==============================
plt.figure(figsize=(10, 5))
plt.plot(terminal_replay_size_history, label="Terminal Replay Buffer Size")
plt.xlabel("Update")
plt.ylabel("Replay Buffer Size")
plt.title("Terminal Replay Buffer Size Curve")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(save_dir, "terminal_replay_size_curve.png"))
plt.close()
# ==============================
...
print(
    f"[Terminal Replay Summary] size={terminal_replay.size()} | "
    f"window={terminal_window} | "
    f"batch={terminal_replay_batch} | "
    f"epochs={terminal_replay_epochs}"
)
# ==============================
