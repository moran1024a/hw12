...
def run_one_episode(model, model_path, seed, render=False, log_dir="./test_logs"):
    """
    每个 episode 单独创建一个环境，确保 seed 真正生效。
    这样不会出现“同一个地图测 20 次”的问题。
    """
...
def main():
    ...
    test_episodes = 30              # 建议至少 30；更稳一点可以 50 / 100
    render = True                  # 批量评估建议 False；想肉眼看时再开 True
    log_dir = "./test_logs"
